...
function make_entry.gen_from_packages(opts)
	...
	local make_display = function(module_name)
		...
		local display = string.format("%-" .. opts.column_len .. "s : %s", module_name,
			vim.fn.fnamemodify(p_path, ":~:."))
	...
	local signs = (function()
		...
		for _, severity in ipairs(type_diagnostic) do
			local status, sign = pcall(function()
				...
				return vim.trim(vim.fn.sign_getdefined("DiagnosticSign" .. severity:lower():gsub("^%l", string.upper))
				[1].text)
			end)
			if not status then
