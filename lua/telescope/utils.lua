...
local path_filename_first = function(path, reverse_directories, is_dir, dir_hl, coordinates)
  ...
  table.insert(path_style, { { #transformed_path - #tail, #transformed_path }, "TelescopeResultsComment" })
...
utils.transform_path = function(opts, path, coordinates)
  ...
  if display_opts.shorten then
    transformed_path = path_shorten(transformed_path, display_opts.shorten.len, display_opts.shorten.exclude)
  end
...
utils.create_path_display = function(entry, opts)
