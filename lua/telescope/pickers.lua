...
function Picker:find ()
  ...
  local on_lines = function (_, _, _, first_line, last_line)
    ...
    self.manager = pickers.entry_manager(self.max_results, vim.schedule_wrap(function (index, entry)
          ...
          local set_ok = pcall(vim.api.nvim_buf_set_lines, results_bufnr, row, row + 1, false, {
              display
            });
          ...
          if not set_ok then
            ...
            vim.api.nvim_buf_set_lines(results_bufnr, row, row + 1, false, {
                display
              });
          end
        end));
 ...
 -- print("DETACH:", ...)
  ...
  local on_buf_leave = string.format([[  autocmd BufLeave <buffer> ++nested ++once :lua require('telescope.pickers').on_close_prompt(%s)]], prompt_bufnr);
  ...
  if prompt_border_win then
    vim.api.nvim_win_set_option(prompt_border_win, 'winhl', 'Normal:TelescopeNormal');
  end
  if results_border_win then
    vim.api.nvim_win_set_option(results_border_win, 'winhl', 'Normal:TelescopeNormal');
  end
  if preview_border_win then
    vim.api.nvim_win_set_option(preview_border_win, 'winhl', 'Normal:TelescopeNormal');
  end
...
function Picker:set_selection (row)
  ...
  if self._selection_row then
    ...
    if old_selection then
      a.nvim_buf_set_lines(results_bufnr, self._selection_row, self._selection_row + 1, false, {
          '  ' .. old_selection:sub(3)
        });
    end
  end

  a.nvim_buf_set_lines(results_bufnr, row, row + 1, false, {
      '> ' .. (a.nvim_buf_get_lines(results_bufnr, row, row + 1, false)[1] or ''):sub(3)
    });

  a.nvim_buf_clear_namespace(results_bufnr, ns_telescope_selection, 0, -1);
  a.nvim_buf_add_highlight(results_bufnr, ns_telescope_selection, 'TelescopeSelection', row, 0, -1);
  ...
  if status.preview_win and self.previewer then
    self.previewer:preview(entry, status);
  end
end
