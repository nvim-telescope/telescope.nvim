...
internal.builtin = function(opts)
	...
	if opts.title==nil then
     opts.title = "Telescope Builtin"
	end

	for k, v in pairs(require "telescope.builtin") do
		if opts.filter_match == nil or (k:match(opts.filter_match) ~= nil) then
	...
	internal.jumplist = function(opts)
		...
		for i = #jumplist, 1, -1 do
			if vim.api.nvim_buf_is_valid(jumplist[i].bufnr) then
				jumplist[i].text = vim.api.nvim_buf_get_lines(jumplist[i].bufnr, jumplist[i].lnum - 1, jumplist[i].lnum, false)
				[1]
				or ""
